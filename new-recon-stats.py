...
def run_cloud_enum(entries, workers=5, delay=0.1, debug=False):
    ...
    def _run(word):
        ...
        try:
            ...
            if "no valid templates" in msg.lower():
                raise RuntimeError("nuclei cloud templates not found")
            print(f"[!] nuclei cloud scan for '{word}' exited with {exc.returncode}: {msg}")
        finally:
            if delay>0:
                time.sleep(delay)
    ...
    try:
        with ThreadPoolExecutor(max_workers=max(1,workers)) as executor:
            future_map={executor.submit(_run,word):word for word in entries}
            iterator=as_completed(future_map)
            if HAS_ALIVE:
                with alive_bar(total,title="cloud enum",enrich_print=False) as bar:
                    for fut in iterator:
                        fut.result()
                        bar()
            else:
...
def main():
    ...
    parser.add_argument("--cloud-delay",type=float,default=0.2,help="Delay in seconds between nuclei runs to avoid throttling (default 0.2)")
